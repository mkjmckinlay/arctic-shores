...
with open(os.path.join(PROJECT_PATH, 'files/candidates.csv')) as csv_file:
    ...
    for row in csv_reader:
        ...
        if len(candidate_reference) == 8 and re.match("^[A-Za-z0-9_-]*$", candidate_reference):
            if 0 <= candidate_score <= 100:
                ...
                print(f'reference - {candidate_reference} score - {candidate_score} added')
            else:
                print(f'reference - {candidate_reference} score - {candidate_score} is invalid ')
        else:
